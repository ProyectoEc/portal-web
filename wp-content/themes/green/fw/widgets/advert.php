 ...
 */
add_action( 'widgets_init', 'green_widget_advert_load' );

/**
 * Register our widget.
 */
function green_widget_advert_load() {
	register_widget( 'green_widget_advert' );
}

/**
 * Twitter Widget class.
 */
class green_widget_advert extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_advert', 'description' => esc_html__('Advertisement block', 'green') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'green_widget_advert' );

		/* Create the widget. */
		parent::__construct( 'green_widget_advert', esc_html__('ThemeREX - Advertisement block', 'green'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_advert_inner">
			...
			if ($advert_code!='') {
				echo force_balance_tags(green_substitute_all($advert_code));
			}
			?>
		</div>
		<?php
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => esc_html__('Advertisement block', 'green') );
		$instance = wp_parse_args( (array) $instance, $defaults ); 
		$title = isset($instance['title']) ? $instance['title'] : '';
		$advert_image = isset($instance['advert_image']) ? $instance['advert_image'] : '';
		$advert_link = isset($instance['advert_link']) ? $instance['advert_link'] : '';
		$advert_code = isset($instance['advert_code']) ? $instance['advert_code'] : '';
        $alt = basename($advert_image);
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_image' )); ?>"><?php esc_html_e('Image source URL:<br />(leave empty if you paste advert code)', 'green'); ?></label>
            ...
            <?php
			echo trim(green_show_custom_field($this->get_field_id( 'advert_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'advert_image' )), null));
			if ($advert_image) {
			?>
	            <br /><br /><img src="<?php echo esc_url($advert_image); ?>" style="max-width:220px;" alt="<?php echo esc_html($alt); ?>" />
			<?php
			}
			?>
		</p>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_link' )); ?>"><?php esc_html_e('Image link URL:<br />(leave empty if you paste advert code)', 'green'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_code' )); ?>"><?php esc_html_e('or paste Advert Widget HTML Code:', 'green'); ?></label>
