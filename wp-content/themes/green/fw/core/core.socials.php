<?php
/**
 * AncoraThemes Framework: social networks
 *
 * @package	green
 * @since	green 1.0
 */
...
// Theme init
if (!function_exists('green_socials_theme_setup')) {
	add_action( 'green_action_before_init_theme', 'green_socials_theme_setup' );
	function green_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'green_facebook_og_tags', 5 );
		}
...
$GREEN_GLOBALS['share_links'] = array(
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=green',
...
if (!function_exists('green_add_share_link')) {
	function green_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		global $GREEN_GLOBALS;
		$GREEN_GLOBALS['share_links'] = array_merge( $GREEN_GLOBALS['share_links'], $soc );
	}
}

// Return (and show) share social links
if (!function_exists('green_show_share_links')) {
	function green_show_share_links($args) {
		if ( green_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'type' => 'list',					// share block style: list|block|drop
			'popup' => true,					// open share url in new window or in popup window
			'counters' => green_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => green_get_custom_option('show_share'),				// share block direction
			'caption' => green_get_custom_option('share_caption'),				// share block caption
			'share' => green_get_theme_option('share_buttons'),					// list of allowed socials
		...
		foreach ($args['share'] as $soc) {
			...
			$sn = (($args['style']=='icons') && (!green_strrpos( $sn, '.' ))) ? green_substr( $sn, green_strrpos( $sn, '-' ) + 1 ) : green_substr( $sn, 0, -4);
			if (($pos=green_strrpos($sn, '_'))!==false)
				$sn = green_substr($sn, 0, $pos);
			$url = empty($soc['url']) && !empty($GREEN_GLOBALS['share_links'][$sn]) ? $GREEN_GLOBALS['share_links'][$sn] : $soc['url'];
			...
			$output .= '<div class="sc_socials_item">'
				...
				. ($args['type']=='drop' ? '<i>' . trim(green_strtoproper($sn)) . '</i>' : '')
				. '</div>';
		}
		$output .= '</div>';
...
if (!function_exists('green_prepare_socials')) {
	function green_prepare_socials($list, $style='icons') {
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || green_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : green_get_socials_url(basename($soc['icon']));
				$sn = basename($soc['icon']);
				// $sn = $style=='icons' ? green_substr($sn, green_strrpos($sn, '-')+1) : green_substr($sn, 0, green_strrpos($sn, '.'));
                    $sn = (($style == 'icons') && (!green_strrpos( $sn, '.' ))) ? green_substr( $sn, green_strrpos( $sn, '-' ) + 1 ) : green_substr( $sn, 0, -4);
				if (($pos=green_strrpos($sn, '_'))!==false)
					$sn = green_substr($sn, 0, $pos);
                $alt = basename($icon);
				...
				$output .= '<div class="sc_socials_item">'
						...
						. '>'
						. ($style=='icons' ? '<span class="icon-' . esc_attr($sn) . '"></span>' : ($style=='images' ? '<img src="'.esc_url($icon).'" alt="'.esc_html($alt).'" />' : '<span class="sc_socials_hover" style="background-image: url('.esc_url($icon).');"></span>'))
						. '</a>'
						. '</div>';
			}
		}
...
if (!function_exists('green_get_twitter_data')) {
	function green_get_twitter_data($cfg) {
		$data = get_transient("twitter_data_".($cfg['mode']));
		if (!$data) {
			require_once( green_get_file_dir('lib/tmhOAuth/tmhOAuth.php') );
			...
			$code = $tmhOAuth->user_request(array(
				'url' => $tmhOAuth->url(green_get_twitter_mode_url($cfg['mode']))
			));
			if ($code == 200) {
				$data = json_decode($tmhOAuth->response['response'], true);
				if (isset($data['status'])) {
					$code = $tmhOAuth->user_request(array(
						'url' => $tmhOAuth->url(green_get_twitter_mode_url($cfg['oembed'])),
						'params' => array(
		...
		} else if (!is_array($data) && green_substr($data, 0, 2)=='a:') {
...
if (!function_exists('green_get_twitter_mode_url')) {
	function green_get_twitter_mode_url($mode) {
		$url = '/1.1/statuses/';
...
if (!function_exists('green_prepare_twitter_text')) {
	function green_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('green_get_twitter_followers')) {
	function green_get_twitter_followers($cfg) {
		$data = green_get_twitter_data($cfg); 
		return $data && isset($data[0]['user']['followers_count']) ? $data[0]['user']['followers_count'] : 0;
	}
	// Old version
	/*
	function green_get_twitter_followers($account) {
		$tw = get_transient("twitterfollowers");
		if ($tw !== false) return $tw;
		$tw = '?';
		$url = esc_url('https://twitter.com/users/show/'.($account));
		$headers = get_headers($url);
		if (green_strpos($headers[0], '200')) {
			$xml = green_fgc($url);
...
if (!function_exists('green_get_facebook_likes')) {
	function green_get_facebook_likes($account) {
...
function green_facebook_og_tags() {
	global $post;
	if ( !is_singular() || green_get_global('blog_streampage')) return;
...
if (!function_exists('green_get_feedburner_counter')) {
	function green_get_feedburner_counter($account) {
