...
if (!function_exists('trx_utils_get_theme_option')) {
    function trx_utils_get_theme_option($prm) {
        if (function_exists('green_get_theme_option')) {
            return green_get_theme_option($prm);
        }
...
if ( !function_exists( 'trx_utils_enqueue_slider' ) ) {
	function trx_utils_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
			wp_enqueue_style(  'swiperslider', trx_utils_get_file_url('js/swiper/swiper.min.css'), array(), null );
			wp_enqueue_script( 'swiperslider', trx_utils_get_file_url('js/swiper/swiper.jquery.min.js'), array('jquery'), null, true );
		} else if ($engine=='all' || $engine=='elastistack') {
			wp_enqueue_script( 'modernizr', trx_utils_get_file_url('js/elastistack/modernizr.custom.js'), array(), null, true );
			wp_enqueue_script( 'draggabilly', trx_utils_get_file_url('js/elastistack/draggabilly.pkgd.min.js'), array(), null, true );
			wp_enqueue_script( 'elastistack', trx_utils_get_file_url('js/elastistack/elastistack.js'), array(), null, true );
		}
	}
}
...
if ( !function_exists( 'trx_utils_enqueue_popup' ) ) {
	function trx_utils_enqueue_popup($engine='') {
		if ($engine=='pretty') {
			wp_enqueue_style(  'prettyphoto',	trx_utils_get_file_url('js/prettyphoto/css/prettyPhoto.css'), array(), null );
			wp_enqueue_script( 'prettyphoto',	trx_utils_get_file_url('js/prettyphoto/jquery.prettyPhoto.min.js'), array('jquery'), 'no-compose', true );
		} else {
			wp_enqueue_style(  'magnific-popup',trx_utils_get_file_url('js/magnific/magnific-popup.min.css'), array(), null );
			wp_enqueue_script( 'magnific-popup',trx_utils_get_file_url('js/magnific/jquery.magnific-popup.min.js'), array('jquery'), null, true );
		}
	}
}

//  Enqueue WP colorpicker in front-end
if ( !function_exists( 'trx_utils_enqueue_wp_color_picker' ) ) {
	function trx_utils_enqueue_wp_color_picker() {
		...
		wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
			'clear' => __( 'Clear', 'trx_utils' ),
			'defaultString' => __( 'Default', 'trx_utils' ),
			'pick' => __( 'Select Color', 'trx_utils' ),
			'current' => __( 'Current Color', 'trx_utils' ),
		)); 
...
if ( !function_exists( 'trx_utils_enqueue_googlemap' ) ) {
	function trx_utils_enqueue_googlemap() {
		if (trx_utils_is_on(themerex_get_theme_option('api_google_load'))) {
...
if ( !function_exists( 'trx_utils_enqueue_select2' ) ) {
	function trx_utils_enqueue_select2() {
		wp_enqueue_style(  'select2', trx_utils_get_file_url('js/select2/select2.min.css'), array(), null );
		wp_enqueue_script( 'select2', trx_utils_get_file_url('js/select2/select2.min.js'), array('jquery'), null, true );
	}
}
...
if (!function_exists('trx_utils_get_file_dir')) {
	function trx_utils_get_file_dir($file, $return_url=false) {
		...
		$theme_dir = get_template_directory().'/'.TRX_UTILS_PLUGIN_BASE.'/';
		$theme_url = get_template_directory_uri().'/'.TRX_UTILS_PLUGIN_BASE.'/';
		$child_dir = get_stylesheet_directory().'/'.TRX_UTILS_PLUGIN_BASE.'/';
		$child_url = get_stylesheet_directory_uri().'/'.TRX_UTILS_PLUGIN_BASE.'/';
		$dir = '';
		if (file_exists(($child_dir).($file)))
			$dir = ($return_url ? $child_url : $child_dir) . trx_utils_check_min_file($file, $child_dir);
		else if (file_exists(($theme_dir).($file)))
			$dir = ($return_url ? $theme_url : $theme_dir) . trx_utils_check_min_file($file, $theme_dir);
		else if (file_exists(TRX_UTILS_PLUGIN_DIR . ($file)))
			$dir = ($return_url ? TRX_UTILS_PLUGIN_URL : TRX_UTILS_PLUGIN_DIR) . trx_utils_check_min_file($file, TRX_UTILS_PLUGIN_DIR);
		return apply_filters( $return_url ? 'trx_utils_get_file_url' : 'trx_utils_get_file_dir', $dir, $file );
	}
}
...
if (!function_exists('trx_utils_check_min_file')) {
	function trx_utils_check_min_file($file, $dir) {
		if (substr($file, -3)=='.js') {
			if (substr($file, -7)!='.min.js' && trx_utils_is_off(themerex_get_theme_option('debug_mode', false, false))) {
			...
			if (substr($file, -8)!='.min.css'  && trx_utils_is_off(themerex_get_theme_option('debug_mode', false, false))) {
				$dir = trailingslashit($dir);
	...
	function trx_utils_fpc($file, $data, $flag=0) {
		...
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				...
				if (trx_utils_is_on(themerex_get_theme_option('debug_mode', false, false)))
